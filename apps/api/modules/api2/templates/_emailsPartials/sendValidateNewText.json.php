<?php use_helper('I18N', 'Url') ?>
<?php echo __(<<<EOM
    ¡%WELCOME% a LealTag!

    %FULLNAME%,

    Gracias por registrarte, a partir de este segundo empezarás a ser premiado por
    visitar los sitios que te gustan. Primero valida tu cuenta en este link %1% 

    Si tienes algún inconveniente siguiendo el link anterior, copia y pega la dirección siguiente en la
    barra de direcciones de tu navegador:

    %1%

    Ahora sólo debes bajarte la aplicación desde tu Blackberry,
    iPhone o Android en http://www.lealtag.com/descarga

    Busca el regalito LealTag que identifica a los comercios y marcas afiliadas para empezar
    a ganar. Y si tus locales y marcas preferidas no forman parte de LealTag, pídeles que te
    premien.

    Para más información o si no cuentas con un teléfono móvil, entra a
    nuestra página web http://www.lealtag.com para acceder a tu cuenta, ver tus
    premios e imprimir tu tarjeta LealTag.

    Con tu cuenta de LealTag también puedes ingresar a http://www.tudescuenton.com
    y conseguir los mejores descuentos de tu ciudad. Utiliza el mismo correo electrónico y contraseña 
    al momento de ingresar en http://www.tudescuenton.com 
    y gana tags por comprar cupones y ahorrar.

    Tus comentarios y sugerencias nos ayudan a mejorar, así que cualquier cosa
    escríbenos a info@lealtag.com y síguenos en Twitter @lealtag
    para enterarte de todas las cosas nuevas que vamos a traer para ti.

    Mereces ser premiado

    El equipo de LealTag ;)

    Dirección: Av. Neverí, Quinta Mireille, Colinas de Bello Monte, Caracas - Venezuela.
    Recibió este correo porque usted está registrado en LealTag
EOM
        , array(
    "%1%" => $route1,
    '%FULLNAME%' => $fullname,
    '%WELCOME%' => (strcasecmp($gender, 'female') == 0 ? 'Bienvenida' : 'Bienvenido')
        )
)
?>