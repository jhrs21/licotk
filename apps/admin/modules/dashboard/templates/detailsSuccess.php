...
<div class="form-fields">
    <form action="#" method="post">
        ...
        <a href="<?php echo url_for('@detailsAffiliate?affiliate_id=') . $aff_id ?>">Reiniciar</a>
    </form>
    <a href="<?php echo url_for('homepage') ?>"><< Volver</a>
</div>
<table id="general-stats">
    ...
    <tbody>
        ...
        <tr>
            <td>assets</td>                  <td><?php echo $assets->count() ?></td>  <td>0</td>  <td>web email</td>      <td><?php echo $tag_list['web_email'] ?></td>      <td>0</td>  <td></td>           <td></td>                                   <td></td>
        </tr>
        <tr>
    ...
    <tbody>
        <?php foreach ($assets as $key => $asset): ?>
            <?php if (($key % 2) != 0): ?>
                ...
                <?php if ($asset['color'] == 'Rojo'): ?><td style="background-color:red"><?php echo $asset['color'] ?></td><?php endif; ?>
                <?php if ($asset['color'] == 'Verde'): ?><td style="background-color:green"><?php echo $asset['color'] ?></td><?php endif; ?>
                <?php if ($asset['color'] == 'Amarillo'): ?><td style="background-color:yellow"><?php echo $asset['color'] ?></td><?php endif; ?>
                <!-- FIN COLOR -->
                <td><?php echo $asset['name'] ?></td>
                <td><?php echo $asset['subscriptions'] ?></td>
                <td><?php echo $asset['tickets'] ?></td>
                <td><?php echo $asset['exchanged'] ?></td>
                <td><?php echo $asset['category'] ?></td>
                <td><?php if($asset['subscriptions']==0){echo 0;}else {echo number_format((($asset['subscriptions'] - $asset['pre_registered']) / $asset['subscriptions']) * 100, 2, '.', ''); }?></td>
                <td><a href="<?php echo url_for('@analyticsAsset?affiliate_id=' . $aff_id . "&asset_id=" . $key) ?>">Analytics</a></td>
            </tr>
            <!--<tr class="extra" mine="<?php echo $key; ?>" style="display: none"><td>extra <?php echo $key; ?></td></tr>-->
        <?php endforeach; ?>
    </tbody>
</table>
<script type="text/javascript">
    ...
    $(document).ready(function(){        
        ...
        $("button").click(function () {
            $("tr.extra[mine="+ $(this).attr('mine')+"]").toggle("slow");
        });
