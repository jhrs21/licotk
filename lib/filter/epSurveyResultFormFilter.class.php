 ...
 */
class epSurveyResultFormFilter extends SurveyApplicationFormFilter {

    public function configure() {
        parent::configure();
        
        if (!($this->getOption('survey') instanceof Survey)) {
            throw new InvalidArgumentException("You must pass a Survey object as an option to this filter form!");
        } else {
            $survey = $this->getOption('survey');
        }
        ...
        $this->widgetSchema['asset_id'] = new sfWidgetFormDoctrineChoice(
                array(
                    ...
                    'query' => Doctrine::getTable('Asset')->addBySurveyQuery($survey->getId()),
                    'expanded' => true,
                    'multiple' => true,
                    'label' => false,
                )
