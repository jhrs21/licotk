 ...
 */
class sfValidatorIdNumber extends sfValidatorString
{
    /**
    ...
    protected function doClean($value)
    {
        ...
        if (is_array($value))
        {
            $value = $this->convertIdNumberArrayToString($value);
        }
   ...
   * @return string
   */
    protected function convertIdNumberArrayToString($value)
    {
        // all elements must be empty or a number or one letter
        foreach (array('code', 'number') as $key)
        {
            if (isset($value[$key]) && (!preg_match('/^[[:digit:]]+$/', $value['number']) || !preg_match('/^[A-Z]$/', $value['code'])) && !empty($value[$key]))
            {
